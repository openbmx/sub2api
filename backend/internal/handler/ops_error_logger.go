...
func classifyOpsErrorLog(c *gin.Context, errType, message, code string, status int) (phase string, isBusinessLimited bool, errorOwner string, errorSource string) {
	...
	//
	// Gated on upstreamError, not effectiveUpstreamError: a genuine upstream
	// failure still means the provider was actually hit, so it keeps its own
	// classification even when a local model-configuration refusal coincides.
