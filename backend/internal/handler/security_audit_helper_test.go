...
// A block is a policy decision, not a service fault, so runSecurityAudit must
// mark it for ops error classification on every path that can produce one.
// The WebSocket turn-dedupe path returns early and is the easy one to miss:
// merging upstream's dedupe rewrite left it unmarked, which silently put WS
// blocks back in the SLA error bucket.
func TestRunSecurityAuditMarksBlockOnEveryPath(t *testing.T) {
	gin.SetMode(gin.TestMode)
	blockOnce := func() *turnCountingEngine {
		return &turnCountingEngine{
			mode:      securityaudit.ModeBlocking,
	...
	payload := []byte(`{"type":"response.create","response":{"input":"blocked"}}`)
	subject := middleware2.AuthSubject{UserID: 7}

	t.Run("websocket turn", func(t *testing.T) {
		c := newContext()
		c.Set(securityAuditWSTurnContextKey, 2)
		decision := runSecurityAudit(c, nil, securityaudit.NewCoordinator(nil, blockOnce()), nil, nil, subject,
			"openai_responses", "gpt-test", payload, "subsequent_turn")
		require.NotNil(t, decision)
		require.Equal(t, securityaudit.DecisionBlock, decision.Kind)
		require.True(t, hasSecurityAuditBlock(c), "a WebSocket block must be marked for ops error classification")
	})
	...
	t.Run("http request", func(t *testing.T) {
		...
		require.NotNil(t, decision)
		require.Equal(t, securityaudit.DecisionBlock, decision.Kind)
		require.True(t, hasSecurityAuditBlock(c))
	})

	t.Run("allow is not marked", func(t *testing.T) {
		c := newContext()
		c.Set(securityAuditWSTurnContextKey, 2)
		engine := &turnCountingEngine{mode: securityaudit.ModeBlocking}
		decision := runSecurityAudit(c, nil, securityaudit.NewCoordinator(nil, engine), nil, nil, subject,
			"openai_responses", "gpt-test", payload, "subsequent_turn")
		require.NotNil(t, decision)
		require.True(t, decision.AllowNextStage)
		require.False(t, hasSecurityAuditBlock(c), "only a deliberate block may be marked")
	})
}
