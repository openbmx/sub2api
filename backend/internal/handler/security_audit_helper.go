...
// markSecurityAuditBlock records a deliberate rejection under
// securityAuditBlockedContextKey. Every path in runSecurityAudit that produces
// a Decision must call it, including the ones that return early: upstream keeps
// growing new short-circuits here (the WebSocket turn dedupe, the legacy
// content-moderation bridge), and a path that skips the mark silently puts
// blocks back in the SLA error bucket. Centralized so the invariant is one
// named call a merge conflict can point at, rather than an `if` to re-add by
// hand at each return.
func markSecurityAuditBlock(c *gin.Context, decision securityaudit.Decision) {
	if c == nil || decision.Kind != securityaudit.DecisionBlock {
		return
	}
	// Matching on the message is not an option: the block text is
...
func runSecurityAudit(c *gin.Context, reqLog *zap.Logger, coordinator *securityaudit.Coordinator, legacy *service.ContentModerationService, apiKey *service.APIKey, subject middleware2.AuthSubject, protocol, model string, body []byte, stage string) *securityaudit.Decision {
	...
	if coordinator == nil {
		...
		markSecurityAuditBlock(c, decision)
		...
		if turnNo, ok := securityAuditWSTurn(c); ok {
			...
			if cached, exists := c.Get(securityAuditWSDedupeContextKey); exists {
				if entry, ok := cached.(securityAuditWSDedupeEntry); ok &&
					...
					// No-op while only allows are cached below; kept so the
					// invariant still holds if that cache policy ever widens.
					markSecurityAuditBlock(c, decision)
