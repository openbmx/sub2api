...
// deepseek/minimax）与 opencode 的原值，其他值一律归一为 openai。调度器据此对账号与
// 请求做精确平台匹配：kimi 分组请求只命中 kimi 账号，语义与 openai/grok 一致。
// 漏掉任一平台的后果是它被归一成 openai，于是该平台的分组会命中 openai 账号、
// openai 分组也会命中它——两边互相串号。
// （upstream 曾将本函数改为未导出 normalizeOpenAICompatiblePlatform，本分支的
// handler 调度入口仍需导出，保持导出名。）
func NormalizeOpenAICompatiblePlatform(platform string) string {
	switch platform {
	case PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax, PlatformOpenCode:
		return platform
	default:
