...
type Group struct {
	...
	// 按模型叠乘的额外计费倍率，在分组有效倍率与高峰因子之上再乘；key 支持精确名与末尾 * 前缀通配，精确优先、其次最长前缀
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldVideoModelPrices, group.FieldModelRateMultipliers, group.FieldModelPricing, group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig, group.FieldCodexModelsManifestConfig, group.FieldReasoningEffortMappings:
			values[i] = new([]byte)
