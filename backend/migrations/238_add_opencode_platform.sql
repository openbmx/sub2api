...
--
-- 与 224/226/227/237 同型：DROP IF EXISTS 后重建超集约束，存量行瞬时校验通过。
--
-- OpenCode 不是国产供应商，但与它们共用「account_mode × api_protocol × 分协议
-- base_url」这套接入抽象，所以在平台枚举上并列（见 service.IsCNProvider 注释）。
    ...
    IF monitor_constraint_def IS NULL OR position('opencode' IN monitor_constraint_def) = 0 THEN
        ...
        ALTER TABLE channel_monitors
            ...
            CHECK (provider IN ('openai', 'anthropic', 'gemini', 'grok',
                                'antigravity', 'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode'));
    END IF;
